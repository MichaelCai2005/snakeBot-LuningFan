...
kNpeg = 0;%1;
multSin = 0.1; % soil model sin term prefactor
multCos = 5.0; % soil model cos term prefactor
...
load('/home/arman/Repos/Git/snake/matlab/ExperimentalData/positionAndForceDataOnPeg.mat');
...
for i_dt=1:size(dt,2)
    myLegend{i_dt}=strcat('dt=',num2str(dt(i_dt)));
            ...
            for i_recSpeed=1:size(recoverySpeed,2)
                for i_env=1:size(colEnvelop,2)
                    for i_mar=1:size(colMargine,2)
                        tic
                        system(['./SnakeProject ', num2str(KK), ' ', num2str(AA),  ' ', num2str(ww), ' ',...
                            ...
                            num2str(dt(i_dt)),' ', num2str(iter(i_iter)),' ',num2str(timeIntegrationType(i_tIntType)), ' ',...
                            num2str(recoverySpeed(i_recSpeed)),' ',num2str(colEnvelop(i_env)),...
                            ' ',num2str(colMargine(i_mar)), ' ', num2str(multSin), ' ', num2str(multCos), ...
                            ' ', num2str(xOnePeg), ' ', num2str(zOnePeg)]);
                        fTime = toc
                        ...
                        xdata{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar} = x;
                        zdata{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar} = z;
                        mTimeData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar} = mTime(1,:); % they are all the same
                        id = reshape(id,numSegments,size(movdata,1)/numSegments);

                        [fxSnake,fzSnake,fxPegs,fzPegs,idSnake,idPegs,fTime] = importForceData('','snake.cot',RFTstartTime,dt(i_dt)); 
                        fxSnakeData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar}=fxSnake;
                        fzSnakeData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar}=fzSnake;
                        fxPegsData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar}=fxPegs;
                        fzPegsData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar}=fzPegs;
                        fTimeData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar}=fTime;
                        if(i_dt==1)
                            myTitle{i_iter,i_tIntType,i_recSpeed,i_env,i_mar}= ...
                                strcat('iter=',num2str(iter(i_iter)), ' integType=',num2str(timeIntegrationType(i_tIntType)), ...
                                ' recSp=',num2str(recoverySpeed(i_recSpeed)), ' env=',num2str(colEnvelop(i_env)),...
                                ' mar=',num2str(colMargine(i_mar)));
                        end
                    end
                end
            end
            ...
            for i_env=1:size(colEnvelop,2)
                for i_mar=1:size(colMargine,2)
                    ...
                    for i_dt=1:size(dt,2)
                        ...
                        fxPegsData{i_dt,i_iter,i_tIntType,i_recSpeed,i_env,i_mar}(pegSegmentPlot,:),plotColor{i_dt});
                    end
                    hold off
                    legend(myLegend);
                    title(myTitle{i_iter,i_tIntType,i_recSpeed,i_env,i_mar});
