...
load('C:\Users\Arman\Documents\Repos\Git\snake\matlab\ExperimentalData\experimentalRobot_7thComp_swapped.mat');
    ...
    for AA = .61:.03:.73
            for lx = .056:.002:.062
                    for RFTstartTime = 2.6:.05:2.8
                        ...
                        %%
                        paramsAndMinimums(counter,1:6) = [minSumDelP1, minSumDelP2, minSumDelP3, AA, lx, RFTstartTime];                                               
                    end
            end
    end
...
%% fine tune
    for AA = AA_best3-.01:.02:AA_best3+.01
            for lx = lx_best3-.001:.0005:lx_best3+.001
                    for RFTstartTime = RFTstartTimeBest3-.025:.01:RFTstartTimeBest3+.025
                        counter = counter + 1;
                        [xs,zs,~] = runChrono(KK,AA,ww,0,0,0,stopTime,0,RFTstartTime,0,kNseg,lx,ly,lz,0,0, dt, n_iter, integratorType, recoverySpeed, col_envelop, col_margin, multSin, multCos);
                        sumDelP_InitPos = 1e9;
                        tMinDiff_InitPos = 3;
