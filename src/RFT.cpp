...
RFTSystem::RFTSystem(double mSinMult, double mCosMult) :
ydir_(0., 1., 0.),
xdir_(1., 0., 0.),
ffac_(1.),
sinMult (mSinMult),
...
#ifdef USE_IRR
RFTSystem::RFTSystem(irrlicht::ChIrrApp *app, double mSinMult, double mCosMult) :
		RFTSystem(mSinMult, mCosMult)
{
	mApp = app;
}
#endif
...
#ifdef USE_IRR
void DrawVector(irrlicht::ChIrrApp *mApp, const ChVector<>& pos, const ChVector<>& foc, double scale, int color)
{
    auto driver = mApp->GetVideoDriver();
...
void RFTSystem::ForceHu(double deltah, double cospsi, double sinpsi, double area, double *fnorm, double *fpara)
{
...
//
void RFTSystem::ForceSand(double deltah, double cospsi, double sinpsi, double area, double *fnorm, double *fpara)
{
    // prefac = k * rho * g * z * A * reduction_factor
    double prefac = 2.5 * 2470 * 9.81 * deltah * area * 5e-4;
	*fpara = prefac * sinpsi * sinMult;// 0.2; // 0.2 matches snakebot (9 segment, 8 motor snake -- April 2016)
    const double tan2gamm0 = 0.060330932472924;
//    *fnorm = prefac * cospsi * (1 + 1.8 / sqrt(tan2gamm0 + cospsi * cospsi)); //change 1.8 to 18 to remove  anisotropy (slip = 0)
    *fnorm = prefac * cospsi * cosMult;

}

void RFTSystem::ForceBB(double deltah, double cospsi, double sinpsi, double area, double *fnorm, double *fpara)
{
