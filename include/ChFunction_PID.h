...
namespace chrono
{
    ...
    public:
        ...
        double Get_y(double curr_t) const
        {
            ...
            // trapezoidal method
			double accumulateError = mAccuError + 0.5 * (currError + mLastError) * dt;

			double Out = P * currError + I * accumulateError + D * (currError - mLastError) / dt;
            if (Max > Min)
            {
                // set the limit
		...
		void Update(double curr_t) {
			...
			mLastCalled = curr_t;
			mLastValue = Get_y(curr_t);
		}

		ChFunction* Clone() const {return new ChFunction_PID(P, I, D, NULL); }
